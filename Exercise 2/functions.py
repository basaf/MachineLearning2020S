...
def ridge(X: np.array, y: np.array, test_size=0.2, random_state=1,
        list_alpha=[0, 0.5, 1, 5, 10, 50, 100], scaling: bool = True,
        validation_methods=['holdout', 'cross-validation'],
        baselines=['stratified', 'uniform'],
        path: str = None, filename: str = None):
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, random_state=random_state)
    ...
    index = pd.MultiIndex.from_product([list_alpha, scalings,
        validation_methods, ['Classifier'] + ['Baseline ' + i for i in baselines]],
        names=['alpha', 'scaling', 'validation method', 'classifier'])
    evaluation = pd.DataFrame(index=index)
    # Add empty entries for fit and score times (mean, SD)
    for entry1 in ['fit', 'score']:
        for entry2 in ['MEAN', 'SD']:
        ...
        for s in scalings:
            ...
            for method in validation_methods:
                ...
                if method == 'holdout':
                    ...
                    performance = (check_performance_holdout(ridge, xtest, y_test, y_pred_ridge,
                                                                os.path.join(path, '_'.join(
                                                                    [filename, str(alpha), s, method]))))
                    score_time = process_time() - tic

                    # Write fit and score times
                    evaluation.loc[alpha, s, method, 'Classifier']['fit time MEAN'] = fit_time
                    evaluation.loc[alpha, s, method, 'Classifier']['fit time SD'] = 0
                    evaluation.loc[alpha, s, method, 'Classifier']['score time MEAN'] = score_time
                    evaluation.loc[alpha, s, method, 'Classifier']['score time SD'] = 0
                    ...
                    for baseline in baselines:
                        ...
                        for key in dummy_performance.index:
                            if key not in evaluation:
                                evaluation[key] = np.nan
                            evaluation.loc[alpha, s, method, classifier][key] = (
                                dummy_performance.loc[key]['value'])

                elif method == 'cross-validation':
                    performance = check_performance_CV(ridge, x, y, 5, -1,
                        os.path.join(path, '_'.join([filename, str(alpha),
                            s, method])))  # n_jobs=-1 ... use all CPUs
                    ...
                    for baseline in baselines:
                        ...
                        dummy_performance = check_performance_CV(dummy_clf,
                            x, y, 5, -1, os.path.join(path,
                            '_'.join( [filename, str(alpha), s, method,
                                baseline])))  # n_jobs=-1 ... use all CPUs
...
def plot_evaluation_ridge(path: str = None, filename: str = None):

    evaluation = pd.read_hdf(os.path.join(path, filename + '_evaluation.h5'),
        key='evaluation')
    # Read MultiIndex for the loops
    list_alpha = evaluation.index.levels[0].to_list()
    scalings = evaluation.index.levels[1].to_list()
    validation_methods = evaluation.index.levels[2].to_list()
    classifiers = evaluation.index.levels[3].to_list()
    classifiers.sort(reverse=True)
        ...
        # Plot evaluation parameters over parameters of algorithm
          for s in scalings:
            ...
            for pos in range(int(len(evaluation.keys()) / 2)):
                ...
                for idx, classifier in enumerate(classifiers):
                    for j, method in enumerate(validation_methods):
                        MEAN = evaluation.loc[(slice(None), s,                             method, classifier),
                            evaluation.keys()[2 * pos]].to_numpy()
                        SD = evaluation.loc[(slice(None), s,
                            method, classifier),
                            evaluation.keys()[2 * pos + 1]].to_numpy()

                        lines.append(ax[pos].plot(list_alpha, MEAN,
                            marker=marker_cycle[idx],
                            linestyle=linestyle_cycle[j],
                            label=', '.join([classifier, method]).
                                replace('Classifier', 'Classif.').
                                ...
                                replace('cross-validation', 'CV'))[
                                0])
                        ax[pos].fill_between(list_alpha, MEAN - SD, MEAN + SD,
                            alpha=0.3)
                if key == 'efficiency':
                    # Upper limit shall be maximum of times (mean + sd) to
                    # be the same in all figures
...
def plot_accuracy_ridge(path: str = None, filename: str = None):
    evaluation = pd.read_hdf(os.path.join(path, filename + '_evaluation.h5'),
        key='evaluation')
    ...
    for scaling in scalings:
        for weight in weights:
            label = ', '.join([scaling, weight])
            rows = (slice(None), scaling, 'cross-validation', 'Classifier')
            plt.scatter(
                evaluation.loc[rows, ('accuracy MEAN')],
                evaluation.loc[rows, ('accuracy SD')],
                label=label,
...
def plot_efficiency_ridge(path: str = None, filename: str = None):
    evaluation = pd.read_hdf(os.path.join(path, filename + '_evaluation.h5'),
        key='evaluation')
